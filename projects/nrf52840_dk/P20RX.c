           ...
           if (app_vars.flags & APP_FLAG_TIMER){
             switch (app_vars.state) {
                 case APP_STATE_RX:
                     ...
                     stringToSend[i++] = app_vars.packet[0];
                     stringToSend[i++] = app_vars.packet[1];
                     stringToSend[i++] = app_vars.packet[2];
                     stringToSend[i++] = app_vars.packet[3];
                     stringToSend[i++] = app_vars.packet[4];
                     stringToSend[i++] = app_vars.packet[5];
                     stringToSend[i++] = '\r';
                     stringToSend[i++] = '\n';
                     stringToSend[i++] = '\0';
...
void cb_uart_tx_done(void) {
   ...
   if (app_vars.uart_lastTxByteIndex<8) {
       uart_writeByte(stringToSend[app_vars.uart_lastTxByteIndex]);
   } else {
       app_vars.uartDone = 1;
   }
